//! Generate the `CppBase` Rust bindings from `cpp/receiver.hpp` using
...
fn main() {
    ...
    let header = manifest_dir.join("cpp").join("receiver.hpp");

    let outputs = Build::new()
        .header(&header)
        .cpp_std("c++17")
        .out_dir(&out_dir)
        .compile("member_fn_ptr_shims")
        .expect("Build::compile against receiver.hpp");
