...
| `13-ra-class-method-keywords.patch` | v1.13.5 editor parity — parse the class-body method-modifier **keywords** (`virtual` / `override` / `constructor`) in the RA grammar (new SyntaxKinds + `items.rs` handling) so the fork's keyword surface stops producing parse errors and folds/highlights like the `#[…]` attribute form. +syntax tests. |
| `14-ra-class-transparent-base.patch` | v1.13.6 editor parity — transparent base-member resolution for `class`: hir-ty `lower`/`infer` resolve a derived class's inherited fields/methods through `ClassSignature::base` without an explicit `__base.` qualifier, matching the compiler's auto-Deref. `self.base_field` / `self.base_method()` resolve and type-check in the editor. +method-resolution tests. |
