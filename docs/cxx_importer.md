...
| 20  | `const char*` ↔ `&CStr` / `&str` ergonomics layer                            | Labels, tooltips, file paths                                | ✅ shipped — M20 (configurable `cstr_ergonomics: bool` renders byte-int pointers as `*[const|mut] ::core::ffi::c_char`) + **M20.b** (per-method `_cstr` wrapper that takes `&::core::ffi::CStr` for each `*const c_char` slot and forwards via `.as_ptr()`; emits for ctors / static / instance / virtual; mixed-param methods convert only the cstr slots). `&str` / `Option<&CStr>` / return-side `&CStr` tracked as M20.c. |
