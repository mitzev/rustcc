...
| 18  | Default-argument fan-out (max-arity wrapper + documented defaults)          | `void redraw(int delay = 0)`                                | ✅ shipped — count-only doc hint (M18) + per-arity convenience wrappers (M18.b: `_with_defaults` for the all-defaults variant + `_default_<n>` for each partial-drop level) + **M18.c**: the importer constant-evaluates each default via libclang (`clang_Cursor_Evaluate`), so wrappers pass the *real* C++ value (`int buflen = 128*1024` → `131072_i32`; bools, floats, negative ints, unscoped enums likewise). Slots that don't evaluate to a scalar (string literals, expressions over other decls) fall back to M18.b zero-synthesis (int → `0_<repr>`, float → `0.0`, bool → `false`, raw ptr → `null()`/`null_mut()`, unscoped enum → underlying-zero) with an in-body disclosure comment; skips emission entirely when any trailing default-arg type can't be rendered either way (record-by-value, references, function pointers). |
