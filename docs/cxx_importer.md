...
|-----|------------------------------------------------------------------------|-----------------------------------------------------|--------|
| 11  | Free functions + static methods + static data members at TU/namespace scope | `Fl::run()`, `Fl::wait()`, `fl_color(int)`, `fl_message(...)`, `Fl::scheme_` | ✅ shipped — static methods on classes (M11.a) + free functions (M11.b: `FreeFnSet`, `unsafe extern "C++"` block + `pub fn` wrappers, builtin filter) + class-scope static data members (M11.c: `StaticDataSet`, mangled `Symbol::Variable` link names, `pub fn <name>_ptr()` accessors returning raw pointers) |
...
|-----|------------------------------------------------------------------------------|------------------------------------------------------------|--------|
| 15  | Function pointer types + safe-closure callback wrappers                     | `widget->callback(my_func, user_data)` — FLTK is callback-driven | ✅ shipped — M15 (importer lowers `void (*)(int)` and bare function-prototype to `CxxType::Fn(FnSig)`; renderer emits `Option<unsafe extern "C" fn(...) -> ret>`; `::cxx::CxxCallback<F>` runtime helper for the bare-`Fn()` arity) + **M15.b** (per-callback-type wrapper structs: when an alias target is `void(args..., void*)` — the FLTK / GTK callback shape — the emitter generates an `<Alias>_Wrapper<F: Fn(args...)>` struct that boxes a Rust closure and exposes `(fn_ptr, user_data)` ready for the C++ API; aliases without a trailing `void*` user-data slot or with non-`Fn` targets get plain `pub type` only). |
...
| 20  | `const char*` ↔ `&CStr` / `&str` ergonomics layer                            | Labels, tooltips, file paths                                | ✅ shipped — M20 (`cstr_ergonomics: bool` renders byte-int pointers as `*const c_char`) + **M20.b** (`_cstr(&CStr)` wrapper) + **M20.c** (`_str(&str)` wrapper allocating a temp `CString` per slot with panic-on-interior-nul + `_opt_cstr(Option<&CStr>)` wrapper mapping `None` → `null()` for the FLTK nullable-label / nullable-tooltip pattern). Return-side `&CStr` tracked as M20.d. |
| 21  | Bitfield-aware layout in `rustc_abi_cxx`                                    | Some FLTK structs use `unsigned when_:8;`-style fields. Verify `rustc_abi_cxx::layout` handles them; add support if missing. | ✅ shipped — M21 (probe-then-poison) replaced by **M21.b** (real Itanium bit-packing). New `CxxTypeCtx::bitfield_widths` sidecar stores per-`(class, field_idx)` bit width; layout engine packs consecutive same-container bitfields into one allocation unit, opens a fresh AU when the next bitfield's container differs or doesn't fit, closes the AU when a non-bitfield field follows. `RecordLayout` gains `field_bit_offsets: Vec<u8>` + `field_bit_widths: Vec<u64>` parallel to `field_offsets`. Width-zero bitfields force the next AU boundary. Per-field bit-mask accessor emission on the Rust binding side is tracked as M21.c. |
